...
import me.inkerxoe.oishplugin.storagebags.utils.FileUtil.createDirectory
import me.inkerxoe.oishplugin.storagebags.utils.FileUtil.createFile
...
object ConfigUtil {
    ...
    fun getAllFiles(plugin: String, dir: String): ArrayList<File> {
        return getAllFiles(
            File(
                File(OishStorageBags.plugin.dataFolder.parent, File.separator + plugin),
                File.separator + dir
            )
        )
    }
    ...
    fun getFile(plugin: String, file: String): File {
        return File(File(OishStorageBags.plugin.dataFolder.parent, File.separator + plugin), File.separator + file)
    }
    ...
    fun getFileOrNull(plugin: String, file: String): File? {
        return File(File(OishStorageBags.plugin.dataFolder.parent, File.separator + plugin), File.separator + file).let {
            if (!it.exists()) null
            else it
        }
    }
